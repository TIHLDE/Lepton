...
@pytest.mark.django_db
def test_member_cannot_set_different_author_in_reservation(member, bookable_item, sosialen_user):
    client = get_api_client(user=member)
...
def test_user_can_fetch_all_reservations(reservation, member):
    ...
    assert response.data[0]["author_detail"]["user_id"] == first_reservation.author.user_id
    assert response.data[0]["bookable_item_detail"]["id"] == str(first_reservation.bookable_item.id)
