...
# "event_current_group"/"event_new_group" should have one of 3 different values:
# - None -> The event has no connected group/should remove connection to group
# - "same" -> The event is connected to/should be connected to same group as user is member of
# - "other" -> The event is connected to/should be connected to another group as user i member of
permission_params = pytest.mark.parametrize(
    ...
    (
        # Members of admin-groups have access if event.group is None
        ...
        # Members of admin-groups have access if member of the event.group
        ...
        # HS and Index have access if not member of the event.group
        (AdminGroup.HS, None, None, "other", None, 200),
        (AdminGroup.INDEX, None, None, "other", None, 200),
        # HS and Index have access even if not member of new group
        (AdminGroup.HS, None, None, "other", "other", 200),
        (AdminGroup.INDEX, None, None, "other", "other", 200),
        # Members of admin-groups don't have access if not member of the event.group
        ...
        # Members of admin-groups don't have access if not member of new group
        ...
        # Not member of admin group don't have access
        ("Non_admin_group", None, None, "other", None, 403),
        ("Non_admin_group", None, None, None, None, 403),
        # Leaders of committees and interest-groups have access if event.group is None
        ...
        # Leaders of committees and interest-groups have access if has access of the event.group
        ...
        # Leaders of committees and interest-groups don't have access if not has access of the event.group
        ...
        # Leaders of committees and interest-groups don't have access if not has access of new group
        ...
        # Members of committees and interest-groups don't have access even if member of event.group
...
def permission_test_util(
    ...
    user_group_type = (
        group_type
        if group_type
        else get_group_type_from_group_name(user_member_of_group)
    )

    group = None
    if event_current_group == "same":
        group = Group.objects.get_or_create(
            type=user_group_type, name=user_member_of_group,
        )[0]
    elif event_current_group == "other":
        group = GroupFactory()

    event = EventFactory(group=group)
    expected_title = "Title" if expected_status_code == 200 else event.title
    new_group = None
    if event_new_group == "same":
        new_group = Group.objects.get_or_create(
            type=user_group_type, name=user_member_of_group,
        )[0]
        new_group = new_group.slug
    elif event_new_group == "other":
...
def test_create_event_as_admin(permission_test_util):
    ...
    (user, _, new_group, _, expected_status_code, _, _,) = permission_test_util
...
def test_delete_event_as_admin(permission_test_util):
    ...
    # event for another group, and therefore expects 403. In delete we don't change group
    # and the user should therefore be allowed to delete the event
    if event_new_group == "other":
...
def test_retrieve_events_where_is_admin_only_includes_events_where_is_admin(
    ...
    EventFactory(group=hs)
    EventFactory(group=index)
    EventFactory(group=nok)
    EventFactory(group=nok)
    EventFactory(group=sosialen)
    EventFactory(group=promo)
    EventFactory(group=kontkom)
    EventFactory(group=pythons)
    EventFactory()
